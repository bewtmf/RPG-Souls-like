...
namespace DS
{
    public class PlayerLocomotion : MonoBehaviour
    {
        ...
        public void HandleFalling(float delta, Vector3 moveDirection)
        {
            ...
            // fix lỗi nhân vật bị chìm xuống 1 khoảng khi thực hiện các anim
            if (playerManager.isGrounded)
            {
                if (playerManager.isBusy || inputHandler.moveAmount > 0)
                {
                    myTransform.position = Vector3.Lerp(myTransform.position, targetPosition, Time.deltaTime / 0.001f);
                }
                else
                {
